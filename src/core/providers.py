...
def provider_for(role: str, env: dict[str, str] | None = None) -> ProviderConfig:
    ...
    The standardized role-specific keys are the sole runtime contract. Unknown
    environment names are ignored for provider configuration.
    """
    ...
    values = dict(env) if env is not None else load_runtime_env()
